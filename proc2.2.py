...
class Proc2(Procedure):
    def __init__(self):
        ...
        logger = TranslationLogger(logfile=join(main_folder, 'proc2.2.jsonl'))
        ...
        for dm, folder, dm_id in zip(dms, dm_folders, dm_ids):
            for s in segmented:
                ...
                self.tasks[f'{dm_id}-{s}'][cli.model] = task
        ...
        completed = [
            ...
            'europarl-el',
            'europarl-es',
            'europarl-fi'  # fi has not been completed but we re-run all missing pairs together after all tasks have been run!
        ]
        for c in completed:
            del self.tasks[c]
...
if __name__ == '__main__':
    desc = '''Procedure 2.2 Task Manager
